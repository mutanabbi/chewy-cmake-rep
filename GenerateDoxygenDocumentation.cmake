...
function(_gdd_generate_doxyfile)
    set(_gdd_tpl "${PROJECT_BINARY_DIR}/Doxyfile.tpl")

    execute_process(
        COMMAND "${DOXYGEN_EXECUTABLE}" -s -g "${_gdd_tpl}"
        OUTPUT_QUIET
      )
    if(NOT EXISTS "${_gdd_tpl}")
        message(FATAL_ERROR "Doxygen has failed to generate a Doxyfile template")
    endif()
...
#
    ...
    set(_gdd_doxyfile_in "${PROJECT_BINARY_DIR}/Doxyfile.in")
    file(WRITE "${_gdd_doxyfile_in}" ${_gdd_dne_header})

    set(_gdd_doxyfile_defaults "${PROJECT_BINARY_DIR}/DoxygenDefaults.cmake")
    file(WRITE "${_gdd_doxyfile_defaults}" ${_gdd_dne_header})

    # Read Doxygen parameters
    file(STRINGS "${_gdd_tpl}" _gdd_tpl_params REGEX "[A-Z][A-Z0-9_]+ +=.*")
    foreach(_gdd_param IN LISTS _gdd_tpl_params)
        if(_gdd_param MATCHES "([A-Z][A-Z0-9_]+)( +)=( (.*))?")
            if(CMAKE_MATCH_COUNT EQUAL 4)
                file(APPEND "${_gdd_doxyfile_in}" "${CMAKE_MATCH_1}${CMAKE_MATCH_2}= @DOXYGEN_${CMAKE_MATCH_1}@\n")
                file(APPEND "${_gdd_doxyfile_defaults}" "if(NOT DEFINED DOXYGEN_${CMAKE_MATCH_1})\n    set(DOXYGEN_${CMAKE_MATCH_1} ${CMAKE_MATCH_4})\nendif()\n")
            elseif(CMAKE_MATCH_COUNT EQUAL 2)
                file(APPEND "${_gdd_doxyfile_in}" "${CMAKE_MATCH_1}${CMAKE_MATCH_2}= @DOXYGEN_${CMAKE_MATCH_1}@\n")
            else()
                message(AUTHOR_WARNING "Unexpected line format! Code review required!\nFault line: `${_gdd_param}`")
            endif()
        else()
            message(AUTHOR_WARNING "Unexpected line format! Code review required!\nFault line: `${_gdd_param}`")
        endif()
...
function(list_to_doxygen_quoted_strings LIST_VARIABLE)
...
function(generate_doxygen_documentation target)
    ...
    if(NOT _gdd_CONFIG)
        set(_gdd_CONFIG "${PROJECT_BINARY_DIR}/Doxyfile.in")
    endif()
    ...
    if(NOT _gdd_COMMENT)
        set(_gdd_COMMENT "Generate API documentation using")
    endif()
    ...
    foreach(_item DOXYGEN_INPUT DOXYGEN_EXCLUDE)
        list_to_doxygen_quoted_strings(${_item})
    endforeach()

    # Get other defaults from generated file
    include("${PROJECT_BINARY_DIR}/DoxygenDefaults.cmake" OPTIONAL)
    ...
    # Cleanup $build/docs/html on "make clean"
    # TODO Any other dirs?
    set_property(DIRECTORY APPEND PROPERTY ADDITIONAL_MAKE_CLEAN_FILES "${DOXYGEN_OUTPUT_DIRECTORY}/html")
endfunction()
